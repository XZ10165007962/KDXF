...
def read_data():
	...
	data_2 = pd.read_csv("../xfdata/oppo_breeno_round1_data/test.csv")
...
if __name__ == '__main__':
	...
	lag_1 = 12
	lag_2 = 6
	for id in tqdm(list_12):
		data_i = data[data["session_id"] == id]
		for i in range(1, 13):
			data_i_ = data_i[data_i["rank"] >= (14-i)]
			data_i_, feat_col = trans_data(data_i_, lag_1)
			train_data = data_i_[data_i_["rank"] > (14-i)]
	...
	for id in tqdm(list_6):
		data_i = data[data["session_id"] == id]
		for i in range(1, 13):
			data_i_ = data_i[data_i["rank"] >= (14 - i)]
			data_i_, feat_col = trans_data(data_i_, lag_2)
			train_data = data_i_[data_i_["rank"] > (14 - i)]
			train_data.dropna(inplace=True)
			train_data_x = train_data.loc[:, feat_col]
			train_data_y = train_data.loc[:, label_col]
	...
	sum_data.to_csv("../user_data/tmp_data/linear.csv", index=False)